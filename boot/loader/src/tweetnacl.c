...
void randombytes(u8* bytes, u64 size) {
  u64 i;
  FOR(i, size) {
    bytes[i] = 0;
  }
...
static int crypto_hash(u8 *out,const u8 *m,u64 n)
{
  ...
  {
    u64 z[8],bb[8],a[8],w[16],t;
    int ii,j;

    FOR(ii,8) z[ii] = a[ii] = dl64(h + 8 * ii);

    while (n >= 128) {
      FOR(ii,16) w[ii] = dl64(m + 8 * ii);

      FOR(ii,80) {
        FOR(j,8) bb[j] = a[j];
        t = a[7] + Sigma1(a[4]) + Ch(a[4],a[5],a[6]) + K[ii] + w[ii%16];
        bb[7] = t + Sigma0(a[0]) + Maj(a[0],a[1],a[2]);
        bb[3] += t;
        FOR(j,8) a[(j+1)%8] = bb[j];
        if (ii%16 == 15) {
          FOR(j,16) {
      ...
      FOR(ii,8) { a[ii] += z[ii]; z[ii] = a[ii]; }

      m += 128;
      n -= 128;
    }

    FOR(ii,8) ts64(h + 8 * ii, z[ii]);
  }
